...
**Proceed to engineer review gate ONLY when ALL validations pass.**

---

## Step 4.6: Engineer Review Gate

After traceability generation passes, run the engineer-review agent to
cross-reference ALL artifacts for alignment gaps. This is the final quality check
before implementation begins.

### 4.6.1 Spawn Engineer Review Agent

Use the Task tool to invoke the engineer-review agent:

```
Task:
  subagent_type: "engineer-review"
  description: "Engineer review alignment check"
  prompt: |
    Review alignment for feature in directory: {FEATURE_DIR}
    ...
    Cross-reference all artifacts and report alignment findings
    using Red/Yellow/Gray severity classification.
```

### 4.6.2 Parse Findings

After the agent returns its report:

1. Check the **Status** line: `PASS` or `FAIL`
2. Extract **Red** findings (blocking)
3. Extract **Yellow** findings (should address)
4. Extract **Gray** findings (informational)

If **Status is PASS** (no Red findings):
- Log the review results
- Display: "Engineer review passed. Proceeding to approval gate."
- Continue to Step 5

If **Status is FAIL** (Red findings exist):
- Enter the correction loop (Step 4.6.3)

### 4.6.3 Correction Loop
...
WHILE Red findings exist AND ITERATION <= MAX_ITERATIONS:
  ...
  1. Display current iteration:
     "Engineer Review - Correction Iteration [ITERATION]/3"

  2. For EACH Red finding, apply the appropriate fix:
     - "AC has no implementing task" → Add a new task to tasks.md
       covering the missing acceptance criterion
     - "Plan phase has no corresponding task phase" → Add tasks for
       the missing phase to tasks.md
     - "Contract endpoint has no implementing task" → Add a task to
       tasks.md for the missing endpoint
     - "Data model entity has no implementing task" → Add a task to
       tasks.md for the missing entity
     - "Task description doesn't match plan scope" → Update the task
       description in tasks.md to match plan.md

  3. For EACH Yellow finding, apply fix if straightforward:
     - Scope mismatches → Update task descriptions
     - Wrong file paths → Correct paths in tasks.md
     - Architecture gaps → Add notes to task descriptions

  4. Re-run traceability generation (Step 4.5) to update
     traceability.md with the new/modified tasks

  5. Re-run engineer-review agent (same invocation as Step 4.6.1)

  6. ITERATION += 1

  7. Display iteration results:
     "Iteration [ITERATION-1] complete: [N] Red, [N] Yellow, [N] Gray"
```
...
1. **Generate escalation report** at `{FEATURE_DIR}/engineer-review-escalation.md`:

   ```markdown
   # Engineer Review Escalation: [Feature Name]

   Generated: [ISO timestamp]
   Iterations attempted: 3

   ## Persistent Red Findings

   | # | Area | Finding | Evidence | Fix Attempted |
   |---|------|---------|----------|---------------|
   | 1 | [area] | [finding] | [evidence] | [what was tried] |

   ## Recommendation

   These alignment issues could not be resolved automatically.
   Human review is required before implementation can begin.

   Options:
   1. Manually fix the identified gaps and re-run /4_gofer_tasks
   2. Update spec.md or plan.md to remove requirements that cannot
      be tasked
   3. Accept the gaps and proceed (not recommended)
   ```

2. **Display escalation message**:

   ```
   ⚠️  ENGINEER REVIEW ESCALATION
   ═══════════════════════════════════════════════════════════════

   After 3 correction attempts, alignment issues remain unresolved.
   Human review is required before implementation can begin.

   Escalation report: {FEATURE_DIR}/engineer-review-escalation.md

   Red findings remaining: [N]

   Please review the escalation report and either:
   1. Fix the issues manually, then re-run /4_gofer_tasks
   2. Update spec/plan to align with current tasks
   3. Approve proceeding despite gaps (reply "force-approve")

   ═══════════════════════════════════════════════════════════════
   ```

3. **HALT pipeline** — Do NOT proceed to the approval gate (Step 5) or
   `/5_gofer_implement` until the user resolves the escalation.
