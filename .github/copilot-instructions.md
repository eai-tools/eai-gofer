...
SpecGofer is a **self-orchestrating development system** that uses AI agents to implement features from specifications. You're working with a sophisticated multi-layered architecture that eliminates manual workflows by coordinating Claude AI, Playwright testing, and autonomous validation.
...
**Critical Flow**: Extension → Language Server (MCP tools) → Claude Code → Orchestrator → Engineer/Test Agents
...
**Key Pattern**: The extension uses `specKitParser.ts` to extract YAML frontmatter and parse Markdown task lists with dependencies.
...
- Orchestrator picks up response → Runs tests → Validates
See `src/orchestrator/Orchestrator.ts` lines 45-57
...
```
**Critical**: Tasks only start when ALL dependencies are `completed`.
See `Orchestrator.ts` line 90-96 for dependency resolution.
...
**Important**: Extension bundles Language Server during packaging (see `extension/webpack.config.js`).

## Integration Points

### Claude Code Integration
- **Method 1**: VSCode native MCP support (1.102+) → Reads tools from Language Server
...
❌ **Don't** write code without tests - TDD is required
❌ **Don't** manually edit `.vscode/mcp.json` - Extension auto-generates
❌ **Don't** bypass constitution validation - Engineer agent enforces
❌ **Don't** commit `.claude-input.txt` / `.claude-output.txt` - Temp files
❌ **Don't** hardcode paths - Use `workspacePath` from context
...
2. **MCP tools not showing?** Verify VSCode 1.102+, check `.vscode/mcp.json` exists
...
This system is **self-referential** - it was built to build itself. When making changes:
