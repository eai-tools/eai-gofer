...
**Status**: Draft **Input**: User description: "Integrate Claude Code CLI with
SpecGofer terminal for autonomous execution with human monitoring and WhatsApp
escalation"
...
### User Story 1 - Basic Claude Code Execution with Visual Monitoring (Priority: P1)
...
Claude Code launches in external Terminal with the correct spec context and
...
1. **Given** a specification exists in the project, **When** the user clicks the
   ...
   feature branch and launches Claude Code in an external terminal window
...
3. **Given** Claude Code is running, **When** the user clicks the Stop button,
   **Then** the terminal process is terminated cleanly
4. **Given** Claude Code is running, **When** VSCode is closed, **Then** the
   Claude Code terminal is automatically terminated

---

### User Story 2 - Automated Question Response with Constitution Validation (Priority: P2)
...
verifying auto-responses occur when confidence is above 80% and align with
constitution.
...
1. **Given** Claude Code asks a natural language question, **When** SpecGofer
   ...
   **Then** SpecGofer automatically responds
2. **Given** Claude Code asks a question, **When** the suggested response
   violates the project constitution, **Then** the system escalates rather than
   auto-responding
3. **Given** an auto-response is sent, **When** the action is logged, **Then**
   the output channel shows the question, decision, and confidence level

---

### User Story 3 - WhatsApp Escalation for Uncertain Decisions (Priority: P1)
...
1. **Given** Claude Code asks a question, **When** SpecGofer's confidence is
   ...
   sent to the configured phone number
...
### User Story 4 - Learning from Human Decisions (Priority: P3)
...
- Claude Code CLI not installed shows error dialog with installation
  instructions
- Feature branch uncommitted changes are automatically stashed before checkout
- WhatsApp service down retries 3 times then falls back to VSCode dialog
- Multiple simultaneous sessions are queued to run sequentially (one at a time)
- Terminal output uses circular buffer, keeping most recent 10,000 lines
- Terminal crash triggers auto-restart with state recovery from last checkpoint
- Constitution violations always trigger WhatsApp escalation (never
  auto-respond)
- Learned patterns apply context-aware based on task similarity
