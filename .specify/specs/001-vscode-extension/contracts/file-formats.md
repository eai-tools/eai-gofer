...
This document defines the file format specifications for SpecGofer, including GitHub Spec Kit format, MCP configuration, and legacy JSON format.
  ...
  "mcp": {
    "servers": {
      "specgofer": {
        "command": "node",
        "args": [
          "/absolute/path/to/language-server/dist/server.js"
        ],
        "env": {
...
**Parsing**: Extract H3 headings as article titles, content until next H3 as article body.
...
All file formats defined with validation rules and migration mappings. Parsers can be implemented using `gray-matter` (YAML) and regex (tasks).
