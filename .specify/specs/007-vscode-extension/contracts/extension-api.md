...
This document defines the VSCode Extension API contracts for SpecGofer,
including activation events, commands, tree providers, and extension lifecycle.
...
export async function activate(
  ...
  if (hasSpecify) {
    ...
    vscode.window.createTreeView('specgofer.progress', {
      treeDataProvider: progressProvider,
    });
    vscode.window.createTreeView('specgofer.constitution', {
      treeDataProvider: constitutionProvider,
    });
...
**Command ID**: `specgofer.init` **Title**: "SpecGofer: Initialize Repository"
...
- Creates `.specify/specs/` directory (note: specs always at .specify/specs/,
  not root)
...
**Command ID**: `specgofer.refresh` **Title**: "SpecGofer: Refresh Views"
**Icon**: `$(refresh)` **When**: `specgofer.hasSpecify`
...
**Command ID**: `specgofer.migrate` **Title**: "SpecGofer: Upgrade to Spec Kit
Format" **When**: `specgofer.hasLegacySpecs`
...
**Command ID**: `specgofer.checkUpdates` **Title**: "SpecGofer: Check for
Updates" **When**: Always available
...
**Command ID**: `specgofer.updateTemplates` **Title**: "SpecGofer: Update
Templates" **Category**: "SpecGofer"
...
**Command ID**: `specgofer.openSpec` **Title**: (Hidden) **When**: (Internal
only)
...
**View ID**: `specgofer.progress` **Title**: "SpecGofer" **Location**: Activity
Bar (custom icon)
...
**View ID**: `specgofer.constitution` **Title**: "Constitution" **Location**:
Explorer sidebar (below SpecGofer Progress)
...
try {
  ...
  const message =
    error instanceof SpecKitError ? error.message : 'Unknown error';
  vscode.window
    .showErrorMessage(`SpecGofer: ${message}`, 'View Logs')
    .then((selection) => {
...
**Channel Name**: "SpecGofer" **Log Levels**: INFO, WARN, ERROR **Format**:
...
All extension API contracts defined. Developers can implement components
independently following these specifications.
