...
**Branch**: `016-top5-context-gaps` | **Spec**: spec.md
**Rubric Source**: `.specify/research/comprehensive-context-management-rubric.md`

## Summary

Close the 5 highest-impact gaps in the context management rubric by modifying
4 existing files and creating 2 new files. The work extends the existing
...
**Primary Dependencies**: `@anthropic-ai/sdk` (existing), `@google/generative-ai`
(existing), VSCode Extension API
**Testing**: Vitest for unit tests
**Target Platform**: VSCode Extension + Language Server (LSP)
...
- [x] **Test-Driven Development**: Unit tests for every new component and modification
- [x] **No External Service Dependencies**: LLM calls always have non-LLM fallbacks
- [x] **Backward Compatibility**: Cache version migration, interface extension not replacement
- [x] **Security**: No new credential handling; reuses existing ProviderFactory API key retrieval
...
**Goal**: Replace binary masking with 3-tier decay (`full` → `key-points` → `masked`)
...
Replace `masked: boolean` with `decayTier: DecayTier` across the `ObservationEntry`
interface and all consumers.
...
**Consumer updates** (6 external files reference `observation.masked` or `maskedCount`):
...
Key logic:
- Observations at `decayTier: 'full'` with `age >= keyPointsAge` transition to `key-points`
- Observations at `decayTier: 'key-points'` with `age >= ageThresholdTurns` transition to `masked`
...
Each extractor produces a formatted string with `[KEY POINTS from {type}]` header.
...
add delegation-specific directives. The method already receives `utilizationPercent`
and `stage`. Add three tiers:
...
The `generateRecommendations()` method needs access to the current stage profile's
delegation policy. Two options:
...
Modify `post-tool-use.mjs` to produce stdout when utilization exceeds thresholds.
The script already computes `utilizationPercent` in the bridge object (line ~150).
...
Update `.claude/commands/5_gofer_implement.md` to include a periodic health check
instruction:
...
const response = await this.llmProvider.query({
  prompt: `Summarize this research section in ≤150 tokens. Preserve:\n` +
...
```typescript
const response = await this.llmProvider.query({
  prompt: `Create a single-paragraph abstract (≤100 tokens) of these research findings:\n\n` +
...
const apiKey = vscode.workspace.getConfiguration('gofer').get<string>('anthropicApiKey');
if (apiKey) {
  try {
...
The caller in `ContextBuilder.buildContext()` (line 606) already runs in an async
context, so adding `await` is straightforward.

**Important**: If LLM compression is slow, observations that need compression are
queued and compressed in the background. The first call returns deterministic
key-points immediately; subsequent calls find the LLM result cached and upgrade.
...
export type SectionSource = 'research' | 'memory' | 'observation' | 'code' | 'constitution';
