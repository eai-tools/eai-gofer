...
The `MultiSessionBridgeWatcher` also exposes the legacy single-session API for existing consumers:
...
```typescript
interface IContextWindowProvider extends vscode.TreeDataProvider<ContextWindowItem> {
  ...
  onDidChangeTreeData: vscode.Event<ContextWindowItem | undefined | null | void>;
...
**Old path**: `.specify/hooks/context-bridge.json`
**New path**: `.specify/hooks/context-bridge-{session_id}.json`
...
2. Legacy file: `context-bridge.json` (for backward compat with older extension versions)
