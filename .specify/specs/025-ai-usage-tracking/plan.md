...
**Key Innovation**: Hybrid update mechanism combining FileSystemWatcher (<500ms) with polling fallback (5s) for real-time cost awareness during AI-assisted development sessions.
...
|-----------|------|------------------|
| **AIUsageProvider** | `extension/src/aiUsageProvider.ts` (NEW) | TreeDataProvider registration in extension.ts |
...
| **Package.json** | `extension/package.json:284-287` | REPLACE `goferContextWindow` view registration |
...
- **Rate Limiting**: Polling limited to 5s interval (no flood risk)
...
- [ ] Implement periodic polling (5s interval)
  - setInterval to call getUsageData() every 5000ms
  - Only poll if file watcher is inactive (fallback mode)
...
- [ ] Integration test: Polling fallback works when file watcher disabled
...
**Tasks**:
- [ ] Add panel refresh command
  - Register `gofer.refreshAIUsage` command
  - Button in panel toolbar (icon: `$(refresh)`)
  - Calls `aiUsageMonitor.forceRefresh()` → emits 'usage-update'
- [ ] Add error handling
...
extension/
├── src/
│   ├── aiUsageProvider.ts               # NEW - TreeDataProvider for panel
│   ├── autonomous/
│   │   ├── AIUsageMonitor.ts            # NEW - Service for usage aggregation
│   │   ├── CostBudgetEnforcer.ts        # EXTEND - reuse pricing data
│   │   └── MultiSessionBridgeWatcher.ts # INTEGRATE - session detection
│   ├── council/
│   │   └── UsageLogger.ts               # INTEGRATE - read usage data
│   ├── ui/
│   │   ├── AIUsageStatusBar.ts          # NEW - Optional status bar
...
├── package.json                          # MODIFY - view registration (line 284-287)
└── package.nls.json                      # OPTIONAL - localized strings

tests/
├── unit/
│   ├── aiUsageMonitor.test.ts           # NEW - monitor unit tests
│   ├── aiUsageProvider.test.ts          # NEW - provider unit tests
│   └── aiUsageStatusBar.test.ts         # NEW - status bar unit tests
└── integration/
    ├── aiUsagePanel.test.ts             # NEW - panel integration tests
    └── aiUsageUpdates.test.ts           # NEW - real-time update tests
```
...
|------|--------|------------|
| **FileSystemWatcher fails on some platforms** | High - Real-time updates broken | Polling fallback (5s interval) ensures updates continue |
...
| FR2: Real-Time Cost Tracking | COVERED | Phase 2 (FileSystemWatcher + polling, <1s latency) |
...
| FR8: Panel Refresh and Updates | COVERED | Phase 2 (EventEmitter pattern), Phase 6 (Refresh command) |

**Coverage**: 4/4 user stories (100%), 8/8 functional requirements (100%)

### Acceptance Criteria Mapping

| User Story | Acceptance Criterion | Plan Component | Implementation Approach |
|-----------|---------------------|----------------|------------------------|
| US1 | Panel displays current session total cost (<1s latency) | Phase 2: AIUsageMonitor | FileSystemWatcher (<500ms) + polling (5s fallback) |
...
**Coverage**: 19/19 acceptance criteria (100%)
...
3. **Real-Time Updates**: The hybrid FileSystemWatcher + polling approach ensures <1s latency (typically ~200-500ms via file watch) while providing resilience if file watching fails on some platforms.
