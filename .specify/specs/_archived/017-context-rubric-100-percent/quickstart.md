...
Expected: Same 5 pre-existing failures (agent-stop-extraction.test.ts), no new
failures.
...
**Solution**: Check that `incrementTurn()` is being called. Look for
`[Gofer] Turn counter:` in extension output.

### LLM features not activating

**Problem**: Deterministic fallbacks always used **Solution**: Ensure
`gofer.anthropicApiKey` is set in VSCode settings. Check extension output for
`[Gofer] LLM provider initialized`.

### KnowledgeGraph empty after file reads

**Problem**: `loadGraphContext()` returns empty **Solution**: Graph producers
require hook bridge events. Ensure Claude Code is running with hooks enabled.
