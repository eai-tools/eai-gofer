...
- `.claude/commands/6_gofer_validate.md` — Current validation command (570
  lines)
...
- `extension/resources/copilot-prompts/6_gofer_validate.prompt.md` — Copilot
  copy
...
- `tests/e2e/autoCompaction.spec.ts`: 462 lines, 27 test cases, **none test
  anything**
...
- Does NOT check: mutation scores, mock ratios, test authenticity, spec
  traceability
...
- **Prompt-only architecture**: All Gofer commands are markdown prompts, not
  executable code. The validation command instructs Claude what to do — it
  cannot run Stryker or fast-check directly. It instructs Claude to run them.
...
- `extension/src/autonomous/SlopDetector.ts` — existing slop detection code (no
  tests)
- `extension/src/autonomous/CheckpointValidator.ts` — existing checkpoint
  validation (no tests)

## Open Questions

- [ ] Should the rubric point weights be configurable per-project via
      constitution.md?
- [ ] Should remediation iterations count toward the max-3 limit per feature or
      per session?
- [ ] Should the UI/E2E category (10 pts) be redistributed when a feature has no
      UI?
