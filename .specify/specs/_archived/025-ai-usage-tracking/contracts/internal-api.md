...
## API 5: gofer.refreshAIUsage Command

### Command Signature

```typescript
vscode.commands.registerCommand('gofer.refreshAIUsage', async () => {
  await aiUsageMonitor.refresh();
...
Manually trigger AI usage panel refresh. Invoked via toolbar button or command palette to force immediate data reload from usage logs. Provides on-demand updates without waiting for automatic hourly updates.

### Parameters

**None** - Command accepts no parameters

### Returns

**Type**: `Promise<void>`

**Description**: Resolves when refresh operation completes (panel updated)

**Timing**: Completes within 1 second per NFR Performance requirement (line 290 in spec.md)

### Behavior

**Implementation Flow**:

1. User triggers refresh via:
   - Panel toolbar button (icon: `$(sync)`)
   - Command palette: "Gofer: Refresh AI Usage"
2. Command invokes `AIUsageMonitor.refresh()` method
3. Monitor reloads data from `.specify/logs/council-usage.jsonl`
4. Monitor emits `'usage-update'` event
5. Provider receives event and calls `AIUsageProvider.refresh()`
6. TreeView fires `onDidChangeTreeData` → VSCode re-renders panel
7. Loading state indicator shown during refresh (UX requirement)

**Data Sources**:
- `UsageLogger.getUsageSummary()` - Historical usage data
- `CostBudgetEnforcer.getSnapshot()` - Budget status
- `MultiSessionBridgeWatcher` - Active session detection

**Update Guarantee**: Always reloads from disk (never cached), ensuring latest data

**Availability**: Works regardless of automatic update settings (file watch or polling can be disabled)

### Error Handling

**No throws**: Command never throws visible errors to user

**Silent Failures**:
- Log file missing → Returns empty usage data (no error shown)
- Invalid JSONL format → Skips malformed lines, logs warning
- File read errors → Falls back to last known state

**User Feedback**: Loading state indicator provides visual feedback during refresh

### User Stories Served

- **US5**: Manual Panel Refresh (P1) - Core feature implementation
  - Panel toolbar includes refresh button/icon ✅
  - Command palette includes "Gofer: Refresh AI Usage" command ✅
  - Manual refresh updates panel within 1 second ✅
  - Refresh available even when automatic updates disabled ✅
  - Refresh button shows loading state during update ✅

### Related APIs

**Integrates with**:
- `AIUsageMonitor.refresh()` - Service-level refresh method
- `AIUsageProvider.refresh()` - Panel refresh trigger
- `UsageLogger.getUsageSummary()` - Data source
- `CostBudgetEnforcer.getSnapshot()` - Budget data

**Command Registration**:
```typescript
// extension.ts - register command during initialization
context.subscriptions.push(
  vscode.commands.registerCommand('gofer.refreshAIUsage', async () => {
    await state.aiUsageMonitor?.refresh();
    state.aiUsageProvider?.refresh();
  })
);
```
...
**Latency Target**: <1 second (line 290, 334 in spec.md)

**Breakdown**:
- File I/O: ~100-200ms (read council-usage.jsonl)
- JSON parsing: ~50-100ms (parse JSONL entries)
- Aggregation: ~50-100ms (calculate provider totals)
- UI refresh: ~50-100ms (TreeView re-render)
- **Total**: ~300-500ms typical, <1000ms guaranteed

**Resource Usage**:
- Memory: O(n) where n = number of log entries (typically <10MB for 1000 entries)
- Disk I/O: Single file read (no writes)
- CPU: Single-threaded JSON parsing (non-blocking)

### Example Usage

```typescript
// User clicks toolbar refresh button
// Command invoked automatically by VSCode

// Programmatic invocation (testing)
await vscode.commands.executeCommand('gofer.refreshAIUsage');

// Integration with AIUsageMonitor
class AIUsageMonitor extends EventEmitter<UsageUpdateEvents> {
  async refresh(): Promise<void> {
    ...
    this.emit('usage-update', {
      summary,
      budget,
      trigger: 'manual' // Indicates manual refresh
    });
  }
}

// AIUsageProvider receives event
class AIUsageProvider implements vscode.TreeDataProvider<UsageItem> {
  refresh(): void {
    this._onDidChangeTreeData.fire(); // Triggers VSCode TreeView refresh
  }
}
```
...
- Command invokes `AIUsageMonitor.refresh()`
- Monitor emits `'usage-update'` event with `trigger: 'manual'`
- Provider receives event and fires `onDidChangeTreeData`

**Integration Tests**:
- End-to-end: Command → Monitor → Provider → TreeView refresh
- Verify refresh completes within 1 second
- Verify data reloaded from disk (not cached)
- Verify works when automatic updates disabled

**UI Tests**:
- Toolbar button visible in AI Usage panel
- Command appears in command palette
- Loading indicator shown during refresh
- Panel updates after refresh completes
