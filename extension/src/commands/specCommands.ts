...
export function registerSpecCommands(
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('eaiGofer.executeAllPendingSpecs', async () => {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('eaiGofer.hydrateSpec', async (uri: vscode.Uri) => {
      ...
      async (progress) => {
        for (let i = 0; i < executionOrder.length; i++) {
          ...
          try {
            ...
            console.error(`[EAI-GOFER] Failed to execute spec ${spec.id}:`, error);
          }
        }
      }
    );
...
async function executeSpec(spec: Spec): Promise<void> {
  ...
  await vscode.commands.executeCommand('eaiGofer.startAutonomous', spec);
}
