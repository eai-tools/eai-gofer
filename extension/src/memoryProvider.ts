...
export class MemoryProvider implements vscode.TreeDataProvider<MemoryItem> {
  ...
  constructor(workspacePath: string) {
    console.log(`[SpecGofer] MemoryProvider initialized for workspace: ${workspacePath}`);
  ...
  private async loadMemoryDocuments(): Promise<void> {
    try {
      console.log(`[SpecGofer] Loading memory documents from: ${this.memoryPath}`);
      ...
      try {
        ...
        console.log('[SpecGofer] Memory directory does not exist');
        return;
      }
      ...
      console.log(`[SpecGofer] Found ${markdownFiles.length} markdown files in memory`);
      ...
      this.loadError = null;
      console.log(`[SpecGofer] Loaded ${this.documents.length} memory documents`);
    } catch (error) {
      this.loadError = `Failed to load memory documents: ${error}`;
      console.error('[SpecGofer] Error loading memory documents:', error);
    }
  }
