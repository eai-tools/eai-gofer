...
describe('TerminalManager', () => {
  ...
  describe('sendCommand - edge cases', () => {
    it('should throw error when terminal does not exist', async () => {
      await expect(
        terminalManager.sendCommand('non-existent-id', 'test')
      ).rejects.toThrow('Terminal non-existent-id not found');
    });
    ...
    it('should throw error when terminal is not alive', async () => {
      ...
      await expect(
        terminalManager.sendCommand(terminal.terminalId, 'test')
      ).rejects.toThrow(`Terminal ${terminal.terminalId} is not alive`);
    });
  ...
  describe('captureOutput', () => {
    ...
    it('should throw error when capturing output from non-existent terminal', async () => {
      ...
      await expect(outputIterator.next()).rejects.toThrow(
  ...
  describe('restartTerminal', () => {
    ...
    it('should throw error when restarting non-existent terminal', async () => {
      await expect(
        terminalManager.restartTerminal('non-existent-id')
      ).rejects.toThrow('Terminal non-existent-id not found');
    });
  });
